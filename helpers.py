...
def confirm_constant_vals():
    ...
    chunk_prod_ko_thresh = int(response['block_producer_kickout_threshold'])
    if chunk_prod_ko_thresh != CHUNK_PRODUCER_KICKOUT_THRESHOLD:
        print(bcolors.FAIL, "ERR: Chunk producer kickout threshold. Expected: ", CHUNK_PRODUCER_KICKOUT_THRESHOLD, ". Actual: ", chunk_prod_ko_thresh, bcolors.ENDC)
        all_ok = False
